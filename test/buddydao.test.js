...
describe("BuddyDao", function (){
    ...
    it("Lend authorizes trust operations", async function() {
        // mint busd
        await busdAddrress.approve(buddyDaoAddress.address, "100000000000000000000000000")
        await busdAddrress.connect(minter).mint(minter.address, trustAmount);
        await busdAddrress.allowance(minter.address, buddyDaoAddress.address);

        await  buddyDaoAddress.connect(minter).NewTrust(bob.address, alias, busdAddrress.address, fixedRate, trustAmount);
        // get lend data
        const  data = await buddyDaoAddress.GetParticipantIndexData(minter.address, bob.address, 0);
    ...
    it("Lend Remove authorization operation", async function() {
        ...
        const  dataAfter = await buddyDaoAddress.GetParticipantIndexData(minter.address, bob.address, 0);
    ...
    it("Borrower operation borrowing operation", async function() {
        ...
        const  data = await buddyDaoAddress.GetParticipantIndexData(bob.address, minter.address, 0);
